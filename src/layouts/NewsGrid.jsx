...
const NewsGrid = ({ img, title, date, author, id }) => {
  ...
  const hoverClass = "hover:text-[#307ff9] transition duration-300";
  const textClass = "dark:text-white text-center font-bold text-xl";
  ...
  return (
    <Link to={`/news-details/${id}`}>
      <div className="flex flex-col space-y-8 rounded-md shadow-2xl h-[500px] dark:border dark:border-gray-700">
        ...
        <div className="px-4">
          <p className={`${textClass} ${hoverClass}`}>{title}</p>
        </div>

        <button className={`dark:text-white font-medium text-md ${hoverClass}`}>
          {t("read")}
        </button>
      </div>
    </Link>
